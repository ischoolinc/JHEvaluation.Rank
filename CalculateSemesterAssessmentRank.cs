...
namespace JHEvaluation.Rank
{
    public partial class CalculateSemesterAssessmentRank : BaseForm
    {
        ...
        private void btnCacluate_Click(object sender, EventArgs e)
        {
            ...
            // 因為資料量過大會網路逾時，所以分批寫
            Dictionary<string, List<DataGridViewRow>> gradeStudentDict = new Dictionary<string, List<DataGridViewRow>>();
            ...
            foreach (DataGridViewRow row in dgvStudentList.Rows)
            {
                ...
                if (!gradeStudentDict.ContainsKey(gr))
                    gradeStudentDict.Add(gr, new List<DataGridViewRow>());

                gradeStudentDict[gr].Add(row);
            }
            ...
            bkw.DoWork += delegate
            {
                try
                {
                    ...
                    Dictionary<string, List<string>> grStudentDict = new Dictionary<string, List<string>>();
                    ...
                    {
                        foreach (DataGridViewRow row in gradeStudentDict[gr])
                        {
                            #region 單筆學生資料的SQL
                            string studentSql = @"
    SELECT
    ...
    ";
                            ...
                            if (!grStudentDict.ContainsKey(gr))
                                grStudentDict.Add(gr, new List<string>());

                            grStudentDict[gr].Add(studentSql);
                        }
                    }

					//計算科目
                    foreach (string gr in grStudentDict.Keys)
                    {
...
(
  SELECT
    ...
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, subject ORDER BY subject_score ASC) AS grade_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_class_name, subject ORDER BY subject_score ASC) AS class_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag1, subject ORDER BY subject_score ASC) AS tag1_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag2, subject ORDER BY subject_score ASC) AS tag2_row_number
  FROM
    subject_score
  WHERE
    subject IS NOT NULL
...
(
  SELECT
    ...
    , COUNT(student_id) OVER(PARTITION BY rank_grade_year, rank_tag1, domain) AS tag1_count
    , COUNT(student_id) OVER(PARTITION BY rank_grade_year, rank_tag2, domain) AS tag2_count
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, domain ORDER BY domain_origin_score ASC) AS grade_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_class_name, domain ORDER BY domain_origin_score ASC) AS class_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag1, domain ORDER BY domain_origin_score ASC) AS tag1_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag2, domain ORDER BY domain_origin_score ASC) AS tag2_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, domain ORDER BY domain_score ASC) AS grade_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_class_name, domain ORDER BY domain_score ASC) AS class_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag1, domain ORDER BY domain_score ASC) AS tag1_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag2, domain ORDER BY domain_score ASC) AS tag2_row_number
  FROM
    domain_score
  WHERE
    domain IS NOT NULL
...
(
  SELECT
    ...
    , COUNT(student_id) OVER(PARTITION BY rank_grade_year, rank_tag1) AS tag1_count
    , COUNT(student_id) OVER(PARTITION BY rank_grade_year, rank_tag2) AS tag2_count
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year ORDER BY learn_domain_origin_score ASC) AS grade_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_class_name ORDER BY learn_domain_origin_score ASC) AS class_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag1 ORDER BY learn_domain_origin_score ASC) AS tag1_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag2 ORDER BY learn_domain_origin_score ASC) AS tag2_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year ORDER BY learn_domain_score ASC) AS grade_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_class_name ORDER BY learn_domain_score ASC) AS class_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag1 ORDER BY learn_domain_score ASC) AS tag1_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag2 ORDER BY learn_domain_score ASC) AS tag2_row_number
  FROM
    learn_domain_score
  WHERE
    learn_domain_score IS NOT NULL
...
(
  SELECT
    ...
    , COUNT(student_id) OVER(PARTITION BY rank_grade_year, rank_tag1) AS tag1_count
    , COUNT(student_id) OVER(PARTITION BY rank_grade_year, rank_tag2) AS tag2_count
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year ORDER BY course_learn_origin_score ASC) AS grade_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_class_name ORDER BY course_learn_origin_score ASC) AS class_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag1 ORDER BY course_learn_origin_score ASC) AS tag1_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag2 ORDER BY course_learn_origin_score ASC) AS tag2_origin_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year ORDER BY course_learn_score ASC) AS grade_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_class_name ORDER BY course_learn_score ASC) AS class_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag1 ORDER BY course_learn_score ASC) AS tag1_row_number
    , ROW_NUMBER() OVER(PARTITION BY rank_grade_year, rank_tag2 ORDER BY course_learn_score ASC) AS tag2_row_number
  FROM
    course_learn_score
  WHERE
    course_learn_score IS NOT NULL
...
";
                        ...
                        try
                        {
                            ...
                            //DataTable dtq = queryHelper.Select(insertRankSql_Old);
